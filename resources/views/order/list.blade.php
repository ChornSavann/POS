...
<style>
    ...
    .border-top-primary { border-top: 3px solid #3c8dbc !important; }
    .border-top-success { border-top: 3px solid #28a745 !important; }
    .border-top-danger { border-top: 3px solid #dc3545 !important; }
    .stat-card { transition: all 0.3s; border: 1px solid #eee; }
    ...
    .table-custom-admin th {
        padding: 8px 12px !important; /* បន្ថយពី 15px មកត្រឹម 8px */
        font-size: 13px; /* បន្ថយទំហំអក្សរបន្តិច */
    }

    /* កំណត់ទំហំ Column នីមួយៗឱ្យសមស្រប */
    #salesTable th:nth-child(1) { width: 140px; } /* Date */
    #salesTable th:nth-child(2) { width: 120px; } /* Invoice # */
    #salesTable th:nth-child(3) { width: 150px; }   /* Customer (ទុកឱ្យវាទាញតាមជាក់ស្តែង) */
    #salesTable th:nth-child(4),
    #salesTable th:nth-child(5),
    #salesTable th:nth-child(6) { width: 100px; } /* Totals */
    #salesTable th:nth-child(7) { width: 80px; }  /* Status */
    #salesTable th:nth-child(8) { width: 30px; }  /* Actions */


    @media print {
        /* លាក់អ្វីដែលមិនចង់ឱ្យជាប់ក្នុង PDF/Print */
        .btn-group, .sidebar, .navbar, .pagination, .no-print {
...
<style>
    ...
    .pdf-header { display: none; } /* លាក់ក្នុង Web ធម្មតា */

    .pdf-report-style thead th {
        background-color: #1a237e !important; /* ពណ៌ខៀវចាស់ */
        color: white !important;
        text-align: center;
        border: 1px solid #dee2e6;
    }
    ...
    thead th {
        cursor: pointer;
        user-select: none; /* ការពារកុំឱ្យ highlight អក្សរពេលចុចញាប់ */
    }
    thead th:hover {
    ...
    .fa-sort-up, .fa-sort-down {
        opacity: 1 !important;
        color: #ffc107; /* ពណ៌លឿងទុំ ឬពណ៌ដែលបងស្រលាញ់ */
    }
</style>
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-1 gap-2">
                <div class="d-flex align-items-center small">
                    <form method="get" id="pageSizeForm" class="d-flex align-items-center">
                        Show
                        <select name="pageSize" class="form-select form-select-sm mx-2" style="width:70px" onchange="this.form.submit()">
               ...
               <div class="btn-group shadow-sm">
                   ...
                   <button class="btn btn-outline-secondary btn-sm" onclick="downloadPDF()">
                    ...
                    <a href="javascript:void(0)" onclick="printReport('{{ route('orders.printAll') }}')" class="btn btn-outline-secondary btn-sm">
                ...
                <table class="table table-hover table-custom-admin align-middle mb-0" id="salesTable">
                    ...
                    <tbody>
                        @foreach ($orders as $order)
                            ...
                            <tr>
                                <iframe id="printFrame" style="display:none;"></iframe>
                                <td class="small">{{ \Carbon\Carbon::parse($order->order_date)->format('d-M-Y H:i') }}</td>
                               ...
                               <td class="text-end fw-bold">
                                ...
                                <td class="text-center">
                                    <div class="dropdown">
                                        <button class="btn btn-sm shadow-sm border" type="button" data-bs-toggle="dropdown" style="background: white;">
                                            {{-- <i class="fas fa-ellipsis-v text-secondary"></i> --}}
                                            <i class="fa-solid fa-gears"></i>
                                        </button>
                                        <ul class="dropdown-menu dropdown-menu-end shadow border-0 p-1">
                                            <li>
                                                <a class="dropdown-item rounded-2 py-2" href="javascript:void(0)" onclick="printInvoice({{ $order->id }})">
                                            ...
                                            @if (!$order->is_paid)
                                                ...
                                                <li>
                                                   <a class="dropdown-item rounded-2 py-2" href="javascript:void(0)" onclick="forceOpenModal({{ $order->id }})">
                                                        <i class="fas fa-hand-holding-usd me-2 text-warning"></i> Pay Debt
                                                    </a>
                                                </li>
                                            @endif
                                        </ul>
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(res) {
                            if (res.success) {
                                Swal.fire('ជោគជ័យ', 'ការទូទាត់ត្រូវបានរក្សាទុក!', 'success')
                        ...
                        error: function() {
                            Swal.fire('កំហុស', 'មានបញ្ហាបច្ចេកទេសលើ Server', 'error');
                        }
                    });
                }
            });
